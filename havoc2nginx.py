...
## Description: Converts Havoc profiles to Nginx config file format (/etc/nginx/nginx.conf)
...
ua_string  = "UserAgent"
set_uri    = r'"(.*?)"'
pattern_uriline = r'Uris\s*=\s*\[(.*?)\]'
uri_fullline = re.findall(pattern_uriline, contents, re.DOTALL)
# print(uri_fullline)
...
if contents.find(ua_string) == -1:
    ...
    ua       = contents[ua_start:ua_end].split('"')[1]

# Get all profile URIs based on our regex
if len(re.findall(set_uri,uri_fullline[0])) == 0:
    uris = ""
    errors += "[!] No URIs found\n"
    errorfound = True
else:
    uris = re.findall(set_uri,uri_fullline[0])
    # print(uris)
    # exit()
